...
interface Order {
  ...
  status: 'pending' | 'PAID' | 'ACTIVE' | 'EXPIRED' | 'failed' | 'Shipped' | 'Delivered' | 'Cancelled';
}
...
// ─── Product Modal ────────────────────────────────────────────────────────
const ProductModal = ({
  product, onClose, onAdd, isWishlisted, onToggleWishlist, reviews, onAddReview,
}: {
  product: Product; onClose: () => void;
  onAdd: (p: Product, size: string, color: string) => void;
  isWishlisted: boolean; onToggleWishlist: (id: string) => void;
  reviews: Review[]; onAddReview: (r: Review) => void;
}) => {
  const [qty, setQty] = useState(1);
  const [tab, setTab] = useState<'desc'|'materials'|'care'|'reviews'>('desc');
  ...
  const handleHelpful = (reviewId: string) => {
    ...
    saveReviews(updated);
    onAddReview(updated.find(r=>r.id===reviewId)!);
  };
          ...
          <div className="p-6 sm:p-8 flex flex-col overflow-y-auto">
            ...
            {productReviews.length > 0 && (
              <button onClick={()=>setTab('reviews')} className="flex items-center gap-2 mb-3 hover:opacity-70 transition-opacity">
                ...
                <span className="text-[10px] text-brand-charcoal/50 font-bold">{avgRating.toFixed(1)} ({productReviews.length} review{productReviews.length!==1?'s':''})</span>
              </button>
            )}
            ...
            <div className="flex gap-3 border-b border-brand-charcoal/10 mb-4 overflow-x-auto">
              {(['desc','materials','care','reviews'] as const).map(t=>(
                <button key={t} onClick={()=>setTab(t)} className={cn("text-[10px] uppercase tracking-widest font-bold pb-3 border-b-2 transition-colors whitespace-nowrap shrink-0",tab===t?"border-brand-olive text-brand-charcoal":"border-transparent text-brand-charcoal/40")}>
                  {t==='desc'?'Details':t==='reviews'?`Reviews${productReviews.length>0?` (${productReviews.length})`:''}`:t}
                </button>
              ))}
            </div>
...
const OrderStatusBadge = ({status}: {status: Order['status']}) => {
  const map: Record<string,string> = {PAID:'bg-emerald-100 text-emerald-700',pending:'bg-amber-100 text-amber-700',ACTIVE:'bg-blue-100 text-blue-700',EXPIRED:'bg-zinc-100 text-zinc-500',failed:'bg-red-100 text-red-600',Shipped:'bg-purple-100 text-purple-700',Delivered:'bg-teal-100 text-teal-700',Cancelled:'bg-red-50 text-red-400'};
  return <span className={cn('text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-full',map[status]||map.pending)}>{status}</span>;
};

// ─── Orders Drawer ────────────────────────────────────────────────────────
const OrdersDrawer = ({ orders, onClose, onRefreshStatus }: { orders: Order[]; onClose: ()=>void; onRefreshStatus: (id: string)=>Promise<void> }) => {
            ...
            : [...orders].reverse().map(o=>(
              <div key={o.id} className="bg-white border border-brand-charcoal/8 p-5">
                ...
                <div className="text-xs text-brand-charcoal/50 space-y-1 mb-3">
                  ...
                  <p><span className="font-bold text-brand-charcoal/70">Ship to:</span> {o.shipping.city}, {o.shipping.state||o.shipping.country}</p>
                </div>
                <div className="flex gap-2">
