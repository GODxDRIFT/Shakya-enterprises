...
import { Menu, X, ShoppingBag, Search, ChevronDown, Instagram, Facebook, Heart } from 'lucide-react';
...
export const Navbar = ({
  ...
  const handleSearch = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && searchVal.trim()) {
      ...
      // dispatch custom event for catalog to pick up
  ...
  return (
    <nav className={cn(
      ...
      scrolled ? "bg-white/95 backdrop-blur-xl border-b border-brand-charcoal/5 shadow-sm py-3" : "bg-gradient-to-b from-black/60 to-transparent"
    )}>
      ...
      <div className={cn("flex items-center gap-4", c)}>
        <AnimatePresence>
          {searchOpen && (
            <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 160, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="overflow-hidden">
              <input
                ref={sRef} value={searchVal} onChange={e => setSearchVal(e.target.value)} onKeyDown={handleSearch}
        ...
        <button onClick={onOpenWishlist} className="relative hover:text-brand-gold transition-colors">
          ...
          {wishlistCount > 0 && (
            <span className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold shadow-lg">{wishlistCount}</span>
          )}
        </button>
        ...
        <button onClick={onOpenCart} className="relative hover:text-brand-gold transition-colors">
          ...
          {cartCount > 0 && <span className="absolute -top-1.5 -right-1.5 bg-brand-gold text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold shadow-lg">{cartCount}</span>}
        </button>
      </div>
