...
import { Menu, X, ShoppingBag, Search, ChevronDown, Instagram, Facebook, Heart, Package } from 'lucide-react';
...
export const AnnouncementBar = ({ onClose }: { onClose: () => void }) => (
  <div className="bg-brand-charcoal text-white text-[10px] font-bold uppercase tracking-[0.2em] py-2.5 px-4 flex items-center justify-center gap-6 relative">
    ...
    <button onClick={onClose} className="absolute right-4 text-white/50 hover:text-white transition-colors">
      <X size={12} />
    </button>
  </div>
);
...
export const Navbar = ({
  ...
  onOpenWishlist, wishlistCount,
  onOpenOrders, ordersCount,
}: {
  ...
  return (
    <nav className={cn(
      ...
      <div className="absolute left-1/2 -translate-x-1/2 text-center">
        <h1 className={cn("serif text-2xl sm:text-3xl font-bold tracking-widest uppercase transition-colors duration-500", scrolled ? "text-brand-olive" : "text-white")}>
          SHAKYA
        </h1>
        <span className={cn("block text-[9px] tracking-[0.4em] uppercase transition-colors duration-500", scrolled ? "text-brand-charcoal/60" : "text-white/80")}>
          Enterprises
        </span>
      </div>
      ...
      <div className={cn("flex items-center gap-4", c)}>
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 160, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="overflow-hidden">
              <input
                ref={sRef} value={searchVal}
                onChange={e => setSearchVal(e.target.value)}
                onKeyDown={handleSearch}
                placeholder="Search & press Enter"
                className={cn("text-[11px] w-full bg-transparent border-b outline-none pb-1", scrolled ? "border-brand-charcoal/30" : "border-white/40 placeholder:text-white/50")}
              />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <button onClick={onOpenWishlist} className="relative hover:text-brand-gold transition-colors">
          ...
          {wishlistCount > 0 && (
            <span className="absolute -top-1.5 -right-1.5 bg-rose-500 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold shadow-lg">
              {wishlistCount}
            </span>
          )}
        </button>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, x: '-100%' }} transition={{ type: 'tween', duration: 0.4 }}
          ...
          >
            ...
            {/* Mobile: orders & wishlist shortcuts */}
            <div className="mt-8 flex gap-6">
              <button onClick={() => { onOpenWishlist(); setMobileOpen(false); }} className="flex items-center gap-2 text-sm text-brand-charcoal/60 font-bold uppercase tracking-widest">
                <Heart size={16} /> Wishlist {wishlistCount > 0 && `(${wishlistCount})`}
              </button>
              <button onClick={() => { onOpenOrders(); setMobileOpen(false); }} className="flex items-center gap-2 text-sm text-brand-charcoal/60 font-bold uppercase tracking-widest">
                <Package size={16} /> Orders {ordersCount > 0 && `(${ordersCount})`}
              </button>
            </div>
