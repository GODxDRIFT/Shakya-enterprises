...
export const AnnouncementBar = () => {
  ...
  return (
    <div className="bg-brand-charcoal text-white text-[10px] font-bold uppercase tracking-[0.2em] py-2.5 px-4 flex items-center justify-center gap-6 relative z-[60]">
      ...
      <button onClick={() => setV(false)} className="absolute right-4 text-white/50 hover:text-white"><X size={12} /></button>
    </div>
  );
};
        ...
        <AnimatePresence>
          {searchOpen && (
            <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 160, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="overflow-hidden">
              <input ref={sRef} placeholder="Search..." className={cn("text-[11px] w-full bg-transparent border-b outline-none pb-1", scrolled ? "border-brand-charcoal/30" : "border-white/40 placeholder:text-white/50")} />
            </motion.div>
          )}
        </AnimatePresence>
